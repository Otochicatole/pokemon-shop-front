...
const product = { id: 'p1', sku: 'SKU-1', slug: 'card-one', name: 'Card one', description: 'A card', kind: 'SINGLE_CARD' as const, stockMode: 'UNIQUE' as const, price: { amountMinor: '125000', currency: 'USD' as const }, available: 2, productVersion: 3, images: [] };
describe('frontend contracts', () => {
  ...
  it('rejects malformed products', () => { expect(() => productSchema.parse({ ...product, price: { amountMinor: 10, currency: 'USD' } })).toThrow(); });
