...
export function statusLabel(status: string) { return ({ PENDING_PAYMENT: 'Pendiente de pago', PAYMENT_REVIEW: 'En revisión', PAID: 'Pagada', PREPARING: 'Preparando', READY_FOR_PICKUP: 'Lista para retirar', SHIPPED: 'Enviada', COMPLETED: 'Completada', CANCELLED: 'Cancelada', EXPIRED: 'Expirada', REFUND_RECORDED: 'Reembolso registrado', PAYMENT_REQUIRES_REVIEW: 'Requiere revisión' } as Record<string, string>)[status] ?? status; }
