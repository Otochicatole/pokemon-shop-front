...
export const sellerOrderBuyerSchema = z.object({
  ...
  carrier: z.string().nullable().optional(), trackingCode: z.string().nullable().optional(), items: z.array(z.object({ productId: z.string().nullable(), name: z.string(), quantity: z.number().int(), unitPrice: moneySchema, lineTotal: moneySchema })), timeline: z.array(orderTimelineEventSchema).default([]), issues: z.array(z.object({ id: z.string(), status: z.string(), reason: z.string(), createdAt: z.string().or(z.date()) }).passthrough()).default([]),
}).passthrough();
