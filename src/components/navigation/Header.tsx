...
export function Header({ cartCount = 0, sessionSlot, className = '' }: { cartCount?: number; sessionSlot?: ReactNode; className?: string }) {
  ...
  return <header className={`site-header ${className}`}><div className="header-inner"><Link href="/" className="brand" onClick={close}><span className="brand-mark">✦</span><span>Card Shop<small>objetos para coleccionar</small></span></Link><nav className={`main-nav ${open ? 'is-open' : ''}`} aria-label="Navegación principal"><Link href="/" onClick={close}>Inicio</Link><Link href="/catalog" onClick={close}>Catálogo</Link></nav><div className="header-actions"><HeaderSearch value={search} onChange={setSearch} onSubmit={submitSearch} />{sessionSlot ?? <Link href="/account" className="icon-link" aria-label="Mi cuenta"><UserRound size={18} /></Link>}<CartIndicator count={cartCount} /><button className="menu-toggle" aria-label={open ? 'Cerrar menú' : 'Abrir menú'} aria-expanded={open} onClick={() => setOpen(!open)}>{open ? <X size={19} /> : <Menu size={19} />}</button></div></div></header>;
}
