      ...
      {images.map((image, index) => (
        <li key={image.id}>
          ...
          <label>
            ...
            <input key={`${image.id}:${image.altText ?? ''}`} defaultValue={image.altText ?? ''} disabled={disabled} onBlur={(event) => { if (event.target.value !== (image.altText ?? '')) onAltChange(image.id, event.target.value); }} maxLength={255} />
          </label>
          <div>
            <button type="button" disabled={disabled || index === 0} onClick={() => onMove(image.id, -1)} aria-label="Mover imagen a la izquierda"><ArrowLeft size={16} /></button>
            <button type="button" disabled={disabled || index === images.length - 1} onClick={() => onMove(image.id, 1)} aria-label="Mover imagen a la derecha"><ArrowRight size={16} /></button>
            <button type="button" disabled={disabled} onClick={() => onRemove(image.id)} aria-label="Retirar imagen"><Trash2 size={16} /></button>
          </div>
        </li>
      ))}
    </ol>
