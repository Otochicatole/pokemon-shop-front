...
export function AdminShell({ adminName, adminEmail, navigation, onLogout, children }: { adminName: string; adminEmail: string; navigation: AdminNavigationItem[]; onLogout: () => void | Promise<void>; children: ReactNode }) {
  ...
  const labels: Record<string, string> = { products: 'Productos', new: 'Nuevo', inventory: 'Inventario', suppliers: 'Proveedores', orders: 'Órdenes', payments: 'Pagos', fulfillment: 'Envíos', customers: 'Clientes', loyalty: 'Fidelidad', config: 'Configuración', transfer: 'Datos de transferencia', support: 'Soporte', notifications: 'Notificaciones', audit: 'Auditoría' };
