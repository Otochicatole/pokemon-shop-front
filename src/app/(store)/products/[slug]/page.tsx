      ...
      <div className="detail-grid">
        <div className="detail-gallery">
          <div className="detail-main-image">
            {image ? <Image src={image.url} alt={image.altText || product.name} fill priority sizes="(max-width: 900px) 100vw, 55vw" /> : <div className="image-placeholder large"><span>◈</span></div>}
          </div>
        </div>
        <div className="detail-copy">
          <p className="eyebrow">{kindLabel} · {product.sku}</p>
          <h1>{product.name}</h1>
          ...
          <div className="detail-cta">
            ...
            <p>{product.available > 0 ? `${product.available} disponibles · stock verificado al comprar` : 'Actualmente sin stock'}</p>
          </div>
        </div>
      </div>
    </section>
