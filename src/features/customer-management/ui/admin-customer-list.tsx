...
export function AdminCustomerListView() {
  ...
  return <><AdminPageHeader eyebrow="Consulta de identidad" title="Clientes" description="Vista de solo lectura de estado, verificación y actividad comercial." actions={<Button variant="secondary" onClick={() => void query.refetch()}><RefreshCw size={16} />Actualizar</Button>} /><div className={shared.adminToolbar}><TextField className={shared.adminSearchField} label="Buscar" value={filters.search} onChange={(event) => change('search', event.target.value)} placeholder="Nombre o email" /><SelectField label="Estado" value={filters.status} onChange={(event) => change('status', event.target.value)}><option value="">Todos</option><option value="ACTIVE">Activos</option><option value="SUSPENDED">Suspendidos</option></SelectField><SelectField label="Email" value={filters.verified} onChange={(event) => change('verified', event.target.value)}><option value="">Todos</option><option value="true">Verificados</option><option value="false">Sin verificar</option></SelectField></div>{query.isLoading ? <div className={shared.adminLoading}>Cargando clientes</div> : query.isError ? <div className={shared.adminErrorPanel}><div><h2>No pudimos cargar clientes</h2><p>{adminErrorMessage(query.error)}</p></div></div> : <><AdminDataTable rows={query.data?.data ?? []} rowKey={(row) => row.id} caption="Clientes" columns={[
    { key: 'customer', header: 'Cliente', render: (row) => <div><Link className={shared.adminTableLink} href={`/admin/customers/${row.id}`}>{row.name ?? 'Sin nombre'}</Link><small className={shared.adminBlockMuted}>{row.email}</small></div> },
    ...
    { key: 'paid', header: 'Compras', align: 'right', render: (row) => <span className={shared.adminMoney}>{adminMoney(row.paidTotal)}</span> },
    { key: 'joined', header: 'Alta', render: (row) => adminDate(row.createdAt) },
    { key: 'action', header: 'Detalle', align: 'right', render: (row) => <Link className={shared.adminIconButton} href={`/admin/customers/${row.id}`} aria-label={`Abrir cliente ${row.email}`}><Eye size={17} /></Link> },
