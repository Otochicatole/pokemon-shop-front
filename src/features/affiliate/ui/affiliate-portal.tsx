...
function AffiliateLoading() { return <div className={styles.affiliateFeedback}>Cargando información…</div>; }
  ...
  return <>
    ...
    {query.isLoading ? <AffiliateLoading /> : query.isError ? <AffiliateQueryError message={dataError(query.error)} /> : <>
      <section className={[styles.affiliateGrid, styles.affiliateBalanceGrid].filter(Boolean).join(' ')}>
        ...
        <article className={styles.affiliateStat}><span>Disponible</span><strong>{formatMinor(balance?.availableMinor)}</strong><small>Podés solicitarlo ahora</small></article>
      ...
      <section className={[styles.affiliatePanel, styles.affiliateBalanceHistory].filter(Boolean).join(' ')}>
        <div className={styles.affiliatePanelHeading}><div><p className="eyebrow">Historial financiero</p><h2>Actividad de saldo</h2></div></div>
        ...
        {historyTab === 'entries' ? <div id="affiliate-balance-entries" role="tabpanel" aria-labelledby="affiliate-balance-entries-tab" className={styles.affiliateBalanceTabpanel}>
          {!entries.length ? <div className={styles.affiliateEmpty}>Todavía no hay movimientos.</div> : <div className={styles.affiliateLedgerList}>{entries.map((entry) => <div className={styles.affiliateLedgerRow} key={entry.id}><div><strong>{entry.type.replaceAll('_', ' ')}</strong><span>{formatDate(entry.createdAt)}</span></div><strong className={BigInt(entry.amountMinor) >= 0n ? styles.affiliateMoneyPositive : styles.affiliateMoneyNegative}>{BigInt(entry.amountMinor) >= 0n ? '+' : ''}{formatMinor(entry.amountMinor)}</strong></div>)}</div>}
          ...
          {payoutHistory.isLoading ? <AffiliateLoading /> : payoutHistory.isError ? <AffiliateQueryError message={dataError(payoutHistory.error)} /> : !payoutHistory.data?.items.length ? <div className={styles.affiliateEmpty}>Todavía no hay retiros.</div> : <><div className={styles.affiliateLedgerList}>{payoutHistory.data.items.map((item) => <div className={styles.affiliateLedgerRow} key={item.id}><div><strong>{item.status.replaceAll('_', ' ')}</strong><span>{formatDate(item.createdAt)} · Cuenta terminada en {item.destinationLast4 ?? '—'}</span></div><strong>{formatMinor(item.amountMinor)}</strong></div>)}</div><div className={styles.affiliatePagination} aria-label="Paginación de retiros"><Button variant="ghost" onClick={() => setPayoutPage((page) => Math.max(1, page - 1))} disabled={payoutPage === 1}>Anterior</Button><span>Página {payoutPage} de {payoutHistory.data.totalPages}</span><Button variant="ghost" onClick={() => setPayoutPage((page) => page + 1)} disabled={payoutPage >= payoutHistory.data.totalPages}>Siguiente</Button></div></>}
        </div>}
      </section>
    </>}
