...
function AffiliateLayout({ profile, active, children }: { profile: AffiliateProfile; active: ActiveTab; children: React.ReactNode }) {
  ...
  return <div className="affiliate-shell"><aside className="affiliate-sidebar"><div className="affiliate-brand"><span>CS</span><div><strong>Portal afiliado</strong><small>{profile.publicName}</small></div></div><nav aria-label="Portal de afiliado">{visibleTabs.map(({ href, label, key, icon: Icon }) => <Link key={href} href={href} className={active === key ? 'is-active' : ''} aria-current={active === key ? 'page' : undefined}><Icon size={17} />{label}</Link>)}</nav><Link className="affiliate-back-store" href="/"><Store size={16} />Volver a la tienda</Link></aside><main className="affiliate-main">{children}</main></div>;
}
...
function AffiliateHeader({ eyebrow, title, description, action }: { eyebrow: string; title: string; description?: string; action?: React.ReactNode }) {
  return <header className="affiliate-header"><div><p className="eyebrow">{eyebrow}</p><h1>{title}</h1>{description && <p className="affiliate-header-description">{description}</p>}</div>{action && <div className="affiliate-header-action">{action}</div>}</header>;
}

function AffiliateQueryError({ message }: { message: string }) { return <div className="affiliate-feedback is-error" role="alert"><strong>No pudimos cargar esta sección</strong><span>{message}</span></div>; }
function AffiliateLoading() { return <div className="affiliate-feedback">Cargando información…</div>; }
...
export function AffiliatePortal() {
  ...
  if (profile.isLoading) return <div className="affiliate-loading">Cargando portal de afiliados…</div>;
  if (profile.isError || !profile.data) return <main className="affiliate-access-page"><div className="affiliate-access-card"><p className="eyebrow">Portal de afiliados</p><h1>Acceso no disponible</h1><p>{profileErrorStatus === 403 ? 'Tu cuenta todavía no está habilitada como afiliado o fue suspendida. Contactá a la administración para continuar.' : dataError(profile.error)}</p><Link className="button button-secondary" href="/">Volver a la tienda</Link></div></main>;
...
function AffiliateDashboard({ profile, listings, orders, balance, listingsLoading }: { profile: AffiliateProfile; listings: AffiliateListing[]; orders: AffiliateOrder[]; balance?: AffiliateBalance; listingsLoading: boolean }) {
  ...
  return <><AffiliateHeader eyebrow="Central de vendedores" title={profile.publicName} description="Administrá tus publicaciones, prepará las ventas y mantené actualizada la logística de cada producto." action={<span className={`affiliate-status ${active ? 'is-active' : 'is-suspended'}`}>{active ? 'Afiliado activo' : 'Cuenta suspendida'}</span>} /><section className="affiliate-grid" aria-label="Resumen del afiliado"><article className="affiliate-stat"><span>Publicaciones</span><strong>{listingsLoading ? '…' : listings.length}</strong><small>Borradores y publicaciones activas</small></article><article className="affiliate-stat"><span>Ventas activas</span><strong>{orders.length ? activeOrders : '0'}</strong><small>Pedidos pendientes de entrega</small></article><article className="affiliate-stat"><span>Saldo disponible</span><strong>{formatMinor(balance?.availableMinor)}</strong><small>Listo para solicitar retiro</small></article></section><section className="affiliate-panel"><div className="affiliate-panel-heading"><div><p className="eyebrow">Catálogo del afiliado</p><h2>Publicaciones recientes</h2></div>{active && <Link className="button button-primary" href="/affiliate/listings/new"><FilePlus2 size={16} />Nueva publicación</Link>}</div>{listingsLoading ? <AffiliateLoading /> : listings.length === 0 ? <AffiliateEmptyListing editable={active} /> : <div className="affiliate-listing-list">{listings.slice(0, 8).map((listing) => <AffiliateListingRow key={listing.id} listing={listing} editable={active} />)}</div>}<Link className="affiliate-panel-link" href="/affiliate/listings">Ver todas las publicaciones →</Link></section>{active && <section className="affiliate-panel affiliate-next-steps"><div><p className="eyebrow">Para empezar</p><h2>Completá el circuito de venta</h2></div><div className="affiliate-step-list"><Link href="/affiliate/listings/new"><span>01</span><div><strong>Creá una publicación</strong><small>Precio, stock e imágenes claras.</small></div></Link><Link href="/affiliate/logistics"><span>02</span><div><strong>Configurá la logística</strong><small>Definí zonas, tarifas o puntos de retiro.</small></div></Link><Link href="/affiliate/orders"><span>03</span><div><strong>Gestioná tus ventas</strong><small>Actualizá el estado hasta completar la entrega.</small></div></Link></div></section>}</>;
}

function AffiliateSuspendedNotice() { return <><AffiliateHeader eyebrow="Acceso limitado" title="Cuenta suspendida" description="Podés consultar tus publicaciones, ventas y saldo, y terminar entregas ya pagadas. Las nuevas publicaciones, cambios de logística y retiros están bloqueados." /><section className="affiliate-panel"><div className="affiliate-feedback"><strong>Operación restringida</strong><span>Contactá a administración si necesitás revisar la suspensión.</span></div></section></>; }

function AffiliateListingReadOnly({ listing }: { listing: AffiliateListing }) { return <><AffiliateHeader eyebrow="Publicación" title={listing.product.name} description={`Estado actual: ${listingStatusLabel(listing.status)}`} action={<Link className="button button-secondary" href="/affiliate/listings">Volver</Link>} /><section className="affiliate-panel"><div className="affiliate-order-meta"><span>Precio <strong>{formatMinor(listing.product.priceMinor)}</strong></span><span>Stock <strong>{listing.product.inventory?.available ?? 0}</strong></span><span>Versión <strong>{listing.product.version}</strong></span></div><p>{listing.product.description || 'Sin descripción.'}</p>{listing.reviewNote && <p className="affiliate-review-note">Nota de revisión: {listing.reviewNote}</p>}</section></>; }

function AffiliateEmptyListing({ editable = true }: { editable?: boolean }) { return <div className="affiliate-empty"><Package size={28} /><p>Todavía no tenés publicaciones.</p>{editable && <Link href="/affiliate/listings/new">Crear la primera</Link>}</div>; }
...
function AffiliateListingRow({ listing, onSubmit, onArchive, onDelete, busy = false, detailed = false, editable = true }: { listing: AffiliateListing; onSubmit?: () => void; onArchive?: () => void; onDelete?: () => void; busy?: boolean; detailed?: boolean; editable?: boolean }) {
  ...
  return <article className="affiliate-listing-row"><div className="affiliate-listing-copy"><strong>{listing.product.name}</strong><span>{listing.product.kind === 'SEALED_PRODUCT' ? 'Producto sellado' : 'Accesorio'} · {formatMinor(listing.product.priceMinor)} · Stock {listing.product.inventory?.available ?? 0}</span>{detailed && listing.reviewNote && <small className="affiliate-review-note">Nota de revisión: {listing.reviewNote}</small>}</div><span className={`affiliate-listing-status is-${statusClass(listing.status)}`}>{listingStatusLabel(listing.status)}</span>{editable && <div className="affiliate-row-actions"><Link className="button button-ghost" href={`/affiliate/listings/${listing.id}`}><Pencil size={14} />Editar</Link>{canSubmit && onSubmit && <Button variant="secondary" onClick={onSubmit} disabled={busy}><CheckCircle2 size={15} />Enviar a revisión</Button>}{listing.status === 'APPROVED' && onArchive && <Button variant="ghost" onClick={onArchive} disabled={busy}><Archive size={14} />Archivar</Button>}{canSubmit && onDelete && <Button variant="ghost" onClick={onDelete} disabled={busy}><Trash2 size={15} />Eliminar</Button>}</div>}</article>;
}
...
function AffiliateOrdersView({ query, onRefresh }: { query: QueryResult<AffiliateOrder[]>; onRefresh: () => void }) {
  ...
  const rows = query.data ?? [];
  return <><AffiliateHeader eyebrow="Operación comercial" title="Ventas" description="Actualizá la preparación y la entrega. La confirmación final y la liberación del saldo quedan fuera del control del afiliado." action={<Button variant="secondary" onClick={() => void query.refetch()} disabled={query.isFetching}>Actualizar</Button>} />{query.isLoading ? <AffiliateLoading /> : query.isError ? <AffiliateQueryError message={dataError(query.error)} /> : rows.length === 0 ? <section className="affiliate-panel"><div className="affiliate-empty"><ClipboardList size={28} /><p>Todavía no tenés ventas.</p><span>Cuando alguien compre uno de tus productos, aparecerá acá.</span></div></section> : <div className="affiliate-order-list">{rows.map((order) => { const next = nextStatus(order); const cancellable = order.allowedActions.includes('REQUEST_CANCELLATION'); return <article className="affiliate-order-card" key={order.id}><div className="affiliate-order-heading"><div><span className="affiliate-order-number">{order.number}</span><h2>{order.sellerName}</h2></div><span className={`affiliate-listing-status is-${statusClass(order.status)}`}>{orderStatusLabel(order.status)}</span></div><div className="affiliate-order-meta"><span>Subtotal <strong>{formatMinor(order.subtotalMinor)}</strong></span><span>Comisión <strong>{formatMinor(order.commissionMinor)}</strong></span><span>Tu neto <strong className="affiliate-money-positive">{formatMinor(order.sellerNetMinor)}</strong></span></div><div className="affiliate-order-items">{order.items.map((item, index) => <span key={`${order.id}-${index}`}>{item.productName ?? item.name ?? 'Producto'} × {item.quantity}</span>)}</div><div className="affiliate-order-actions"><Link className="button button-ghost" href={`/affiliate/orders/${order.id}`}>Ver detalle</Link>{next && <Button onClick={() => update.mutate({ id: order.id, expectedVersion: order.version, status: next })} disabled={update.isPending}>{next === 'PREPARING' ? 'Empezar a preparar' : next === 'SHIPPED' ? 'Marcar como enviada' : next === 'READY_FOR_PICKUP' ? 'Lista para retirar' : 'Marcar retirada'}</Button>}{cancellable && <Button variant="ghost" onClick={() => { setCancelOpen(cancelOpen === order.id ? null : order.id); setCancelNote(''); }}>Solicitar cancelación</Button>}</div>{cancelOpen === order.id && <form className="affiliate-inline-form" onSubmit={(event) => { event.preventDefault(); if (cancelNote.trim().length < 3) return toast.error('Explicá el motivo de la cancelación'); cancel.mutate({ id: order.id, expectedVersion: order.version, note: cancelNote.trim() }); }}><label>Motivo<textarea value={cancelNote} onChange={(event) => setCancelNote(event.target.value)} minLength={3} maxLength={500} rows={3} required /></label><div className="affiliate-row-actions"><Button type="button" variant="ghost" onClick={() => setCancelOpen(null)}>Volver</Button><Button type="submit" variant="danger" disabled={cancel.isPending}>{cancel.isPending ? 'Enviando…' : 'Enviar solicitud'}</Button></div></form>}</article>; })}</div>}</>;
}
...
function AffiliateOrderDetailView({ order, onRefresh }: { order: AffiliateOrder; onRefresh: () => void }) {
  ...
  return <><AffiliateHeader eyebrow="Detalle de venta" title={order.number} description={`${order.sellerName} · ${orderStatusLabel(order.status)}`} action={<Link className="button button-secondary" href="/affiliate/orders"><ArrowLeft size={16} />Volver a ventas</Link>} /><section className="affiliate-panel"><div className="affiliate-order-meta"><span>Subtotal <strong>{formatMinor(order.subtotalMinor)}</strong></span><span>Comisión <strong>{formatMinor(order.commissionMinor)}</strong></span><span>Neto pendiente <strong className="affiliate-money-positive">{formatMinor(order.sellerNetMinor)}</strong></span></div><div className="affiliate-order-items">{order.items.map((item, index) => <span key={`${order.id}-${index}`}>{item.productName ?? item.name ?? 'Producto'} × {item.quantity}</span>)}</div>{order.status === 'PREPARING' && order.fulfillmentType === 'SHIPMENT' && <div className="affiliate-inline-grid"><label>Transportista<input value={carrier} onChange={(event) => setCarrier(event.target.value)} placeholder="Opcional" /></label><label>Código de seguimiento<input value={trackingCode} onChange={(event) => setTrackingCode(event.target.value)} placeholder="Opcional" /></label></div>}<div className="affiliate-order-actions">{next && <Button onClick={() => update.mutate(next)} disabled={update.isPending}>{update.isPending ? 'Guardando…' : next === 'PREPARING' ? 'Empezar a preparar' : next === 'SHIPPED' ? 'Marcar como enviada' : next === 'READY_FOR_PICKUP' ? 'Lista para retirar' : 'Marcar retirada'}</Button>}{order.status === 'SHIPPED' || order.status === 'PICKED_UP' ? <p className="form-hint">La confirmación final la realiza el comprador o el cierre automático del sistema.</p> : null}</div></section><section className="affiliate-panel"><div className="affiliate-panel-heading"><div><p className="eyebrow">Historial</p><h2>Timeline de entrega</h2></div></div>{order.statusHistory.length ? <ol className="order-timeline">{order.statusHistory.map((event, index) => <li className={index === order.statusHistory.length - 1 ? 'is-current' : ''} key={event.id}><span className="order-timeline-marker" aria-hidden="true" /><div><strong>{orderStatusLabel(event.toStatus)}</strong><span>{formatDate(event.createdAt)}{event.note ? ` · ${event.note}` : ''}</span></div></li>)}</ol> : <p className="affiliate-muted">Todavía no hay eventos.</p>}</section></>;
}
...
function AffiliateLogisticsView({ query, onRefresh }: { query: QueryResult<AffiliateLogistics>; onRefresh: () => void }) {
  ...
  return <><AffiliateHeader eyebrow="Operación comercial" title="Logística" description="Cada publicación necesita al menos una opción de entrega activa antes de poder enviarse a revisión." action={<Button variant="secondary" onClick={() => void query.refetch()} disabled={query.isFetching}>Actualizar</Button>} />{query.isLoading ? <AffiliateLoading /> : query.isError ? <AffiliateQueryError message={dataError(query.error)} /> : <><div className="affiliate-logistics-layout"><section className="affiliate-panel"><div className="affiliate-panel-heading"><div><p className="eyebrow">Envíos</p><h2>Zonas y tarifas</h2></div></div>{!logistics?.zones.length ? <p className="affiliate-muted">Todavía no configuraste zonas de envío.</p> : <div className="affiliate-logistics-list">{logistics.zones.map((zone) => <article className="affiliate-logistics-card" key={zone.id}><div><strong>{zone.name}</strong><span>{zone.provinces.map((province) => province.province).join(' · ')}</span></div>{zone.rates.length ? <ul>{zone.rates.map((rate) => <li key={rate.id}><span>{rate.name}</span><strong>{formatMinor(rate.priceMinor)}</strong></li>)}</ul> : <p className="affiliate-muted">Sin tarifas. Agregá una para habilitar envíos.</p>}</article>)}</div>}</section><section className="affiliate-panel"><div className="affiliate-panel-heading"><div><p className="eyebrow">Nueva configuración</p><h2>Agregar envío</h2></div></div><form className="affiliate-compact-form" onSubmit={(event) => { event.preventDefault(); const parsed = provinces.split(/[,;\n]/).map((province) => province.trim()).filter(Boolean); if (parsed.length === 0) return toast.error('Indicá al menos una provincia'); createZone.mutate({ name: zoneName.trim(), provinces: parsed }); }}><label>Nombre de zona<input value={zoneName} onChange={(event) => setZoneName(event.target.value)} placeholder="CABA y GBA" required /></label><label>Provincias<input value={provinces} onChange={(event) => setProvinces(event.target.value)} placeholder="CABA, Buenos Aires" required /><small>Separalas con coma.</small></label><Button type="submit" disabled={createZone.isPending}>{createZone.isPending ? 'Guardando…' : 'Crear zona'}</Button></form>{logistics?.zones.length ? <form className="affiliate-compact-form affiliate-form-divider" onSubmit={(event) => { event.preventDefault(); const parsed = parseMinor(ratePrice); if (!rateZoneId || !parsed || !rateName.trim()) return toast.error('Completá zona, nombre y precio'); createRate.mutate({ zoneId: rateZoneId, name: rateName.trim(), priceMinor: parsed }); }}><label>Zona<select value={rateZoneId} onChange={(event) => setRateZoneId(event.target.value)}><option value="">Elegí una zona</option>{logistics.zones.map((zone) => <option key={zone.id} value={zone.id}>{zone.name}</option>)}</select></label><label>Nombre de tarifa<input value={rateName} onChange={(event) => setRateName(event.target.value)} placeholder="Envío estándar" required /></label><label>Precio en USD<input type="number" min="0" step="0.01" value={ratePrice} onChange={(event) => setRatePrice(event.target.value)} placeholder="5,00" required /></label><Button type="submit" disabled={createRate.isPending}>{createRate.isPending ? 'Guardando…' : 'Agregar tarifa'}</Button></form> : null}</section></div><section className="affiliate-panel"><div className="affiliate-panel-heading"><div><p className="eyebrow">Retiro</p><h2>Puntos de retiro</h2></div></div>{logistics?.pickupPoints.length ? <div className="affiliate-pickup-list">{logistics.pickupPoints.map((point) => <div key={point.id}><MapPin size={17} /><div><strong>{point.name}</strong><span>{point.address}</span></div></div>)}</div> : <p className="affiliate-muted">Todavía no configuraste puntos de retiro.</p>}<form className="affiliate-inline-grid" onSubmit={(event) => { event.preventDefault(); createPickup.mutate({ name: pickupName.trim(), address: pickupAddress.trim() }); }}><label>Nombre<input value={pickupName} onChange={(event) => setPickupName(event.target.value)} placeholder="Local de Palermo" required /></label><label>Dirección<input value={pickupAddress} onChange={(event) => setPickupAddress(event.target.value)} placeholder="Av. Santa Fe 1234" required /></label><Button type="submit" disabled={createPickup.isPending}>{createPickup.isPending ? 'Guardando…' : 'Agregar punto'}</Button></form></section></>}</>;
}
...
function AffiliateSettingsView({ profile }: { profile: AffiliateProfile }) {
  ...
  return <><AffiliateHeader eyebrow="Cuenta del vendedor" title="Configuración" description="Actualizá cómo te ven los clientes y dónde recibir los retiros." /><section className="affiliate-panel"><form className="affiliate-new-form" onSubmit={submit}><label>Nombre público<input value={publicName} onChange={(event) => setPublicName(event.target.value)} maxLength={120} required /><small>Es el nombre que aparece en el catálogo y en las órdenes.</small></label><label>Teléfono de contacto<input type="tel" value={contactPhone} onChange={(event) => setContactPhone(event.target.value)} maxLength={50} placeholder="Opcional" /></label><label>Cuenta para retiros<input value={payoutAccount} onChange={(event) => setPayoutAccount(event.target.value)} minLength={4} maxLength={500} placeholder={profile.payoutAccountLast4 ? `Dejar vacío para conservar la cuenta terminada en ${profile.payoutAccountLast4}` : 'CBU, alias o cuenta de pago'} /><small>Por seguridad nunca mostramos la cuenta completa. Si la dejás vacía, se conserva la actual.</small></label><div className="affiliate-form-footer"><span>Versión de configuración: {profile.version}</span><Button type="submit" disabled={update.isPending}>{update.isPending ? 'Guardando…' : 'Guardar cambios'}</Button></div></form></section></>;
}
...
function AffiliateNewListing() {
  ...
  return <><AffiliateHeader eyebrow="Nueva publicación" title="Crear producto" description="Guardá un borrador, completá la logística y luego envialo a revisión." action={<Link className="button button-secondary" href="/affiliate/listings"><ArrowLeft size={16} />Volver</Link>} /><section className="affiliate-panel"><form className="affiliate-new-form affiliate-listing-new-form" onSubmit={(event) => { event.preventDefault(); create.mutate(); }}><label>Nombre<input value={name} onChange={(event) => setName(event.target.value)} maxLength={180} placeholder="Ej. Booster sellado" required /></label><label>Descripción<textarea value={description} onChange={(event) => setDescription(event.target.value)} maxLength={5000} rows={5} placeholder="Contale al comprador qué está ofreciendo…" /></label><div className="affiliate-form-grid"><label>Tipo<select value={kind} onChange={(event) => setKind(event.target.value as ListingKind)}><option value="SEALED_PRODUCT">Producto sellado</option><option value="ACCESSORY">Accesorio</option></select></label><label>Precio en USD<input type="number" min="0.01" step="0.01" inputMode="decimal" value={price} onChange={(event) => setPrice(event.target.value)} placeholder="0,00" required /></label><label>Stock<input type="number" min="1" step="1" inputMode="numeric" value={stock} onChange={(event) => setStock(event.target.value)} required /></label></div><label>Imágenes (hasta 8)<input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={(event) => setFiles(Array.from(event.target.files ?? []).slice(0, 8))} /><small>{files.length ? `${files.length} imagen${files.length === 1 ? '' : 'es'} seleccionada${files.length === 1 ? '' : 's'}` : 'Una buena foto es obligatoria para enviar a revisión.'}</small></label><p className="form-hint">Las publicaciones nuevas comienzan como borrador. Para enviarlas a revisión necesitás imagen, stock positivo y al menos una opción de entrega activa.</p><div className="affiliate-form-footer"><Link className="button button-ghost" href="/affiliate">Cancelar</Link><Button type="submit" disabled={create.isPending || !name.trim() || !price}>{create.isPending ? 'Guardando…' : 'Guardar borrador'}</Button></div></form></section></>;
}
...
function AffiliateListingEditor({ listing, onSaved }: { listing: AffiliateListing; onSaved: () => void }) {
  ...
  return <><AffiliateHeader eyebrow="Editar publicación" title={listing.product.name} description={`Estado actual: ${listingStatusLabel(listing.status)} · Versión ${listing.product.version}`} action={<Button variant="secondary" onClick={() => void goBack()} disabled={busy || returning}><ArrowLeft size={16} />{returning ? 'Cargando…' : 'Volver'}</Button>} /><section className="affiliate-panel"><form className="affiliate-new-form affiliate-listing-new-form" onSubmit={(event) => { event.preventDefault(); save.mutate(); }}><label>Nombre<input value={name} onChange={(event) => setName(event.target.value)} maxLength={180} required /></label><label>Descripción<textarea value={description} onChange={(event) => setDescription(event.target.value)} maxLength={5000} rows={6} /></label><div className="affiliate-form-grid"><label>Precio en USD<input type="number" min="0.01" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} required /></label><label>Stock<input type="number" min="0" step="1" value={stock} onChange={(event) => setStock(event.target.value)} required /></label></div><div className="affiliate-form-footer"><span>Editar precio, stock o contenido obliga a una nueva revisión.</span><Button type="submit" disabled={busy}>{save.isPending ? 'Guardando…' : 'Guardar cambios'}</Button></div></form></section><section className="affiliate-panel"><div className="affiliate-panel-heading"><div><p className="eyebrow">Multimedia</p><h2>Imágenes</h2></div><span>{listing.product.images.length}/8</span></div>{listing.product.images.length ? <div className="affiliate-image-grid">{listing.product.images.map((image) => <div className="affiliate-image-card" key={image.id}><Image src={image.url} alt={image.altText ?? listing.product.name} width={120} height={140} unoptimized /><button type="button" onClick={() => remove.mutate(image.id)} disabled={busy} aria-label={`Retirar imagen de ${listing.product.name}`}><Trash2 size={15} /></button></div>)}</div> : <p className="affiliate-muted">Todavía no hay imágenes.</p>}{listing.product.images.length < 8 && <div className="affiliate-upload-row"><input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={(event) => setFiles(Array.from(event.target.files ?? []).slice(0, 8 - listing.product.images.length))} /><Button type="button" variant="secondary" onClick={() => upload.mutate()} disabled={busy || !files.length}><ImagePlus size={16} />Agregar imágenes</Button></div>}</section></>;
}
