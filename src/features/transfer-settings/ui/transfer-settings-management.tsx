...
function TransferSettingsEditor({ settings }: { settings: AdminTransferSettings }) {
  ...
  return <form className={shared.adminForm} onSubmit={(event) => { event.preventDefault(); if (valid) mutation.mutate(); }}>
    ...
    <section className={styles.adminLoyaltyPreview} aria-live="polite">
    ...
    <div className={shared.adminFormFooter}><span>Versión {settings.version} · {enabled && complete ? 'Transferencia activa' : 'Transferencia inactiva'}</span><Button type="submit" disabled={!valid || mutation.isPending}><Banknote size={16} />{mutation.isPending ? 'Guardando…' : 'Guardar configuración'}</Button></div>
  </form>;
}
...
export function TransferSettingsManagementView() {
  ...
  return <>
    ...
    {query.isLoading ? <div className={shared.adminLoading}>Cargando configuración</div> : query.isError || !query.data ? <div className={shared.adminErrorPanel}><div><h2>No pudimos cargar los datos de transferencia</h2><p>{adminErrorMessage(query.error)}</p><Button variant="secondary" onClick={() => void query.refetch()}>Reintentar</Button></div></div> : <TransferSettingsEditor key={query.data.version} settings={query.data} />}
  </>;
}
