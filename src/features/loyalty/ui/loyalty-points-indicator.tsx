...
import styles from './loyalty-points-indicator.module.css';
...
export function LoyaltyPointsIndicator() {
  ...
  return (
    <Link href="/account/points" className={`${styles.pointsLink} points-link`} aria-label={`Puntos disponibles: ${available}`} title="Puntos disponibles">
      <Coins size={17} />
      <span>{available}</span>
    </Link>
  );
