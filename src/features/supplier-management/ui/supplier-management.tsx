...
function SupplierForm({ supplier, onClose, onSaved }: { supplier: Supplier | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <form className={shared.adminDialogForm} onSubmit={form.handleSubmit((values) => mutation.mutate(values))} noValidate>
    ...
    <div className={shared.adminDialogActions}><Button type="button" variant="secondary" onClick={onClose} disabled={mutation.isPending}>Cancelar</Button><Button type="submit" disabled={mutation.isPending}>{mutation.isPending ? 'Guardando…' : supplier ? 'Guardar cambios' : 'Crear proveedor'}</Button></div>
  </form>;
}
...
export function SupplierManagementView() {
  ...
  return <>
    ...
    <div className={[shared.adminToolbar, styles.supplierToolbar].filter(Boolean).join(' ')}><TextField className={shared.adminSearchField} label="Buscar" value={filters.search} onChange={(event) => changeFilter('search', event.target.value)} placeholder="Nombre, contacto, email o teléfono" /><label className="component-field"><span>Estado</span><select value={filters.active} onChange={(event) => changeFilter('active', event.target.value)}><option value="true">Activos</option><option value="">Todos</option><option value="false">Inactivos</option></select></label></div>
    {query.isLoading ? <div className={shared.adminLoading}>Cargando proveedores</div> : query.isError ? <div className={shared.adminErrorPanel}><div><h2>No pudimos cargar los proveedores</h2><p>{adminErrorMessage(query.error)}</p><Button variant="secondary" onClick={() => void query.refetch()}>Reintentar</Button></div></div> : <>
      <AdminDataTable caption="Proveedores" rows={suppliers} rowKey={(row) => row.id} empty="No hay proveedores que coincidan con la búsqueda." columns={[
        { key: 'name', header: 'Proveedor', render: (row) => <div><strong>{row.name}</strong><small className={shared.adminBlockMuted}>Alta: {adminDate(row.createdAt)}</small></div> },
    ...
    <Dialog open={editor !== undefined} onClose={() => setEditor(undefined)} title={editor ? `Editar ${editor.name}` : 'Nuevo proveedor'} description="Solo el nombre es obligatorio; podés completar el resto de la ficha cuando tengas los datos." className={shared.adminWideDialog}>{editor !== undefined && <SupplierForm key={editor?.id ?? 'new'} supplier={editor} onClose={() => setEditor(undefined)} onSaved={() => queryClient.invalidateQueries({ queryKey: ['admin', 'suppliers'] }).then(() => undefined)} />}</Dialog>
