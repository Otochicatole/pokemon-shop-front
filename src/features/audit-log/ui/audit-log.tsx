...
export function AuditLogView() {
  ...
  return <><AdminPageHeader eyebrow="Registro inmutable" title="Auditoría" description="Acciones administrativas con metadata sensible redactada por el backend." actions={<Button variant="secondary" onClick={() => void query.refetch()}><RefreshCw size={16} />Actualizar</Button>} /><div className={[shared.adminToolbar, styles.auditToolbar].filter(Boolean).join(' ')}><TextField label="Administrador (ID)" value={filters.actorId} onChange={(event) => change('actorId', event.target.value)} /><TextField label="Acción" value={filters.action} onChange={(event) => change('action', event.target.value)} placeholder="PRODUCT_UPDATED" /><TextField label="Entidad" value={filters.entityType} onChange={(event) => change('entityType', event.target.value)} /><TextField label="Request ID" value={filters.requestId} onChange={(event) => change('requestId', event.target.value)} /><TextField label="Desde" type="date" value={filters.from} onChange={(event) => change('from', event.target.value)} /><TextField label="Hasta" type="date" value={filters.to} onChange={(event) => change('to', event.target.value)} /></div>{query.isLoading ? <div className={shared.adminLoading}>Cargando auditoría</div> : query.isError ? <div className={shared.adminErrorPanel}><div><h2>No pudimos cargar la auditoría</h2><p>{adminErrorMessage(query.error)}</p></div></div> : <><AdminDataTable rows={query.data?.data ?? []} rowKey={(entry) => entry.id} caption="Eventos de auditoría" columns={[
    { key: 'date', header: 'Fecha', render: (entry) => adminDate(entry.createdAt, true) },
    { key: 'action', header: 'Acción', render: (entry) => <strong className={shared.adminCode}>{entry.action}</strong> },
    ...
    { key: 'entity', header: 'Entidad', render: (entry) => <div>{entry.entityType}<small className={shared.adminBlockMuted}>{entry.entityId?.slice(0, 12) ?? '—'}</small></div> },
    { key: 'request', header: 'Request ID', render: (entry) => <span className={shared.adminCode} title={entry.requestId ?? undefined}>{entry.requestId?.slice(0, 12) ?? '—'}</span> },
    { key: 'meta', header: 'Metadata', align: 'right', render: (entry) => <button type="button" className={shared.adminIconButton} onClick={() => setSelected(entry)} aria-label={`Ver metadata de ${entry.action}`}><Braces size={17} /></button> },
  ]} /><CursorPagination canPrevious={history.length > 0} canNext={Boolean(query.data?.nextCursor)} loading={query.isFetching} onPrevious={() => { const copy = [...history]; setCursor(copy.pop()); setHistory(copy); }} onNext={() => { if (!query.data?.nextCursor) return; setHistory((current) => [...current, cursor]); setCursor(query.data.nextCursor ?? undefined); }} /></>}<Dialog open={Boolean(selected)} onClose={() => setSelected(null)} title="Detalle de auditoría" description={selected ? `${selected.action} · ${adminDate(selected.createdAt, true)}` : undefined} className={shared.adminWideDialog}><pre className={styles.adminJsonView}>{JSON.stringify(selected?.metadata ?? null, null, 2)}</pre></Dialog></>;
}
