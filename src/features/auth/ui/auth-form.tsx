...
export function AuthForm({ mode }: { mode: 'login' | 'register' }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'register') {
        ...
        router.push(returnTo && returnTo.startsWith('/') && !returnTo.startsWith('//') ? returnTo : '/account');
      }
    } catch (err) {
  ...
  return (
    <form className={`${styles.authForm} auth-form`} onSubmit={submit}>
      ...
      <label>
        Email
        <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" placeholder="vos@ejemplo.com" />
      </label>
      <label>
        Contraseña
        <input
      ...
      {error && <p className={`${styles.formError} form-error`} role="alert">{error}</p>}
      ...
      {mode === 'register' && <p className={`${styles.formHint} form-hint`}>Te enviaremos un token para verificar tu email antes de comprar.</p>}
    </form>
  );
}
