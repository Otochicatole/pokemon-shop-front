...
export type ProfileUpdateInput = { name?: string | null; currentPassword?: string; newPassword?: string; confirmPassword?: string };
export async function updateProfile(input: ProfileUpdateInput): Promise<User> {
  const response = await apiFetch('/auth/profile', { method: 'PATCH', body: JSON.stringify(input) }) as { data: { user: unknown } };
