...
import { fullRefundEnvelopeSchema, orderDetailEnvelopeSchema, orderListEnvelopeSchema, orderStatusEnvelopeSchema, sellerOrderStatusEnvelopeSchema, transferReviewEnvelopeSchema, type AdminOrder } from '../domain/contracts';
...
export async function transitionAdminSellerOrder(id: string, expectedVersion: number, status: string, note?: string) { return adminFetch(`/admin/affiliates/seller-orders/${encodeURIComponent(id)}/status`, { method: 'POST', body: JSON.stringify({ expectedVersion, status, note: note?.trim() || 'Actualización logística desde el detalle de la orden.' }) }, sellerOrderStatusEnvelopeSchema); }
