...
function Detail({ order }: { order: AdminOrder }) {
  ...
  return <div className="admin-detail-grid"><div className="admin-detail-stack"><section className="admin-panel"><div className="admin-panel-header"><h2>Ítems ({order.items.length})</h2></div><AdminDataTable rows={order.items} rowKey={(item) => item.id} columns={[{ key: 'item', header: 'Producto', render: (item) => <div className="admin-table-primary">{item.imageUrl ? <Image src={item.imageUrl} alt="" width={46} height={56} unoptimized /> : <span className="admin-table-thumb" />}<div><strong>{item.name}</strong><span>{item.sku}</span></div></div> }, { key: 'price', header: 'Precio', align: 'right', render: (item) => adminMoney(item.unitPrice) }, { key: 'qty', header: 'Cant.', align: 'center', render: (item) => item.quantity }, { key: 'total', header: 'Subtotal', align: 'right', render: (item) => <span className="admin-money">{adminMoney(item.lineTotal)}</span> }]} /></section>
    ...
    <aside className="admin-detail-stack"><section className="admin-panel"><div className="admin-panel-header"><h2>Resumen</h2></div><div className="admin-panel-body"><dl className="admin-definition-list"><dt>Subtotal</dt><dd>{adminMoney(order.totals.subtotal)}</dd><dt>Descuento por puntos</dt><dd>{BigInt(order.totals.discount.amountMinor) > 0n ? `−${adminMoney(order.totals.discount)}` : adminMoney(order.totals.discount)}</dd><dt>Envío</dt><dd>{adminMoney(order.totals.shipping)}</dd><dt>Total</dt><dd className="admin-money">{adminMoney(order.totals.total)}</dd><dt>Expira</dt><dd>{adminDate(order.expiresAt, true)}</dd><dt>Versión</dt><dd>{order.version}</dd></dl></div></section><section className="admin-panel"><div className="admin-panel-header"><h2><Coins size={15} /> Fidelidad</h2></div><div className="admin-panel-body"><dl className="admin-definition-list"><dt>Puntos usados</dt><dd>{order.loyalty.pointsRedeemed}</dd><dt>Estado del canje</dt><dd>{redemptionLabels[order.loyalty.redemptionStatus]}</dd><dt>Descuento aplicado</dt><dd>{adminMoney(order.loyalty.pointsDiscount)}</dd><dt>Puntos generados</dt><dd>{order.loyalty.pointsEarned}</dd><dt>Acreditación</dt><dd>{earnedPointsState(order)}</dd><dt>Versión del programa</dt><dd>{order.loyalty.programVersion ?? '—'}</dd><dt>Monto base de acumulación</dt><dd>{adminMoney(order.loyalty.spendPerPoint)}</dd><dt>Valor por punto</dt><dd>{adminMoney(order.loyalty.pointValue)}</dd></dl></div></section><section className="admin-panel"><div className="admin-panel-header"><h2>Cliente</h2></div><div className="admin-panel-body"><dl className="admin-definition-list"><dt>Nombre</dt><dd>{order.customer.name ?? '—'}</dd><dt>Email</dt><dd><Link href={`/admin/customers/${order.customer.id}`}>{order.customer.email}</Link></dd><dt>Estado</dt><dd>{adminLabel(order.customer.status)}</dd></dl></div></section><section className="admin-panel"><div className="admin-panel-header"><h2>Entrega</h2><AdminBadge value={order.fulfillmentType} /></div><div className="admin-panel-body"><dl className="admin-definition-list">{Object.entries(fulfillment).filter(([, value]) => value !== null && value !== '').map(([key, value]) => <div className="admin-definition-row" key={key}><dt>{adminLabel(key)}</dt><dd>{typeof value === 'object' ? JSON.stringify(value) : String(value)}</dd></div>)}</dl></div></section></aside></div>;
}

function SellerOrdersAdminSection({ order }: { order: AdminOrder }) {
  if (order.sellerOrders.length === 0) return null;
  return <section className="admin-panel admin-seller-orders-panel"><div className="admin-panel-header"><div><span className="admin-panel-kicker">Marketplace</span><h2>Subórdenes por vendedor</h2></div><AdminBadge value={`${order.sellerOrders.length} vendedores`} /></div><div className="admin-panel-body seller-order-admin-list">{order.sellerOrders.map((sellerOrder) => <article className="seller-order-admin-card" key={sellerOrder.id}><div><span className="admin-panel-kicker">{sellerOrder.sellerType === 'AFFILIATE' ? 'Afiliado' : 'Tienda'}</span><h3>{sellerOrder.sellerName}</h3><p>{sellerOrder.number} · {adminLabel(sellerOrder.status)} · {sellerOrder.items.length} productos</p></div><div className="seller-order-admin-card-meta"><AdminBadge value={sellerOrder.status} /><span>{adminMoney(sellerOrder.sellerNet)}</span><Link className="button button-secondary" href={`/admin/affiliates/orders/${sellerOrder.id}`}>Ver detalle</Link></div></article>)}</div></section>;
}
...
export function AdminOrderDetailView({ number }: { number: string }) {
  ...
  return <><AdminPageHeader eyebrow={`Orden // ${order.number}`} title={order.number} description={`${adminDate(order.createdAt, true)} · ${order.customer.email}`} actions={<><Link className="button button-secondary" href="/admin/orders"><ArrowLeft size={16} />Volver</Link><Button variant="secondary" onClick={() => void query.refetch()}><RefreshCw size={16} />Actualizar</Button><AdminBadge value={order.status} /></>} />
    ...
    <Detail order={order} />
    <SellerOrdersAdminSection order={order} />
