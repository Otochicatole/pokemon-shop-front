...
export function PaymentManagementView({ initialQueue = 'TRANSFER_REVIEW' }: { initialQueue?: Queue }) {
  ...
  return <><AdminPageHeader eyebrow="Conciliación" title="Pagos" description="Colas operativas separadas. Las aprobaciones, rechazos y reembolsos se resuelven desde la orden para conservar todo el contexto." actions={<Button variant="secondary" onClick={() => void query.refetch()} disabled={query.isFetching}><RefreshCw size={16} />Actualizar</Button>} />
    ...
    <AdminTabPanel tabsId="payment-tabs" tabId={queue} active className={shared.adminTabsContent}>{query.isLoading ? <div className={shared.adminLoading}>Cargando pagos</div> : query.isError ? <div className={shared.adminErrorPanel}><div><h2>No pudimos cargar la cola</h2><p>{adminErrorMessage(query.error)}</p></div></div> : <AdminDataTable rows={query.data?.data ?? []} rowKey={(row) => row.id} caption="Pagos" empty={queue === 'TRANSFER_REVIEW' ? 'No hay transferencias pendientes de revisión.' : queue === 'MERCADO_PAGO_REVIEW' ? 'No hay pagos de Mercado Pago en revisión.' : 'No hay pagos.'} columns={[
      { key: 'order', header: 'Orden', render: (row) => <div><Link className={shared.adminTableLink} href={`/admin/orders/${row.number}`}>{row.number}</Link><small className={shared.adminBlockMuted}>{adminDate(row.createdAt, true)}</small></div> },
      ...
      { key: 'amount', header: 'Monto', align: 'right', render: (row) => <span className={shared.adminMoney}>{adminMoney(row.payment?.amount ?? row.totals.total)}</span> },
      { key: 'action', header: 'Revisar', align: 'right', render: (row) => <Link className={shared.adminIconButton} href={`/admin/orders/${row.number}`} aria-label={`Revisar pago de ${row.number}`}><ExternalLink size={17} /></Link> },
