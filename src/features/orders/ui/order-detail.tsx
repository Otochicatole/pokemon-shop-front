...
export function OrderDetail({ number }: { number: string }) {
  ...
  if (query.isError || !query.data) {
    return (
      <EmptyState title="Orden no encontrada" description="Puede que el número sea incorrecto o que no tengas acceso.">
        <Link href="/account/orders" className="button button-secondary">Volver a mis órdenes</Link>
      </EmptyState>
    );
  }
  ...
  return (
    <div className={styles.orderPage}>
      <Link href="/account/orders" className={`back-link ${styles.backLink}`}><ArrowLeft size={15} aria-hidden="true" />Volver a mis órdenes</Link>
      ...
      <div className={styles.orderColumns}>
        <section className={styles.orderCard}>
          <div className={styles.cardLabel}>Resumen</div>
          <h2>Productos</h2>
          ...
          <div className={styles.summaryLine}><span>Subtotal</span><strong>{formatMoney(order.totals.subtotal)}</strong></div>
          {BigInt(order.totals.discount.amountMinor) > 0n && (
            <div className={`${styles.summaryLine} ${styles.loyaltyDiscount}`}>
              <span>Descuento por puntos</span>
              <strong>−{formatMoney(order.totals.discount)}</strong>
            </div>
          )}
          <div className={styles.summaryLine}><span>Envío</span><strong>{formatMoney(order.totals.shipping)}</strong></div>
          <div className={styles.summaryTotal}><span>Total</span><strong>{formatMoney(order.totals.total)}</strong></div>
          {(order.loyalty.pointsRedeemed > 0 || order.loyalty.pointsEarned > 0) && (
            <div className={styles.orderLoyalty}>
              <Coins size={18} />
              <div>
                {order.loyalty.pointsRedeemed > 0 && (
                  <p>
                    <strong>{order.loyalty.pointsRedeemed} puntos usados</strong> ·{' '}
                    {order.loyalty.redemptionStatus === 'RESERVED'
                      ? 'reservados hasta acreditar el pago'
                      : order.loyalty.redemptionStatus === 'RELEASED'
                      ? 'devueltos por cierre de la orden'
                      : order.loyalty.redemptionStatus === 'RESTORED'
                      ? 'devueltos por reembolso'
                      : 'canje acreditado'}
                  </p>
                )}
                <p>
                  <strong>{order.loyalty.pointsEarned} puntos por la compra</strong> ·{' '}
                  {order.status === 'REFUND_RECORDED' ? 'revertidos por reembolso' : credited ? 'acreditados' : 'se acreditan al aprobar el pago'}
                </p>
              </div>
            </div>
          )}
        ...
        <section className={styles.orderCard}>
          <div className={styles.cardLabel}>Logística</div>
          ...
          <h2 className={styles.mt}>Pago</h2>
          ...
          {payment?.method === 'MERCADO_PAGO' && payment.mercadoPago?.amount && (
          ...
          {payment?.method === 'MERCADO_PAGO' && ['PENDING_PAYMENT', 'PAYMENT_REVIEW'].includes(order.status) && ['READY', 'RETRY_REQUIRED'].includes(payment.paymentSessionStatus ?? '') && (
            <Button variant="secondary" disabled={busy} onClick={() => void resumeCheckout()}>
              {payment.paymentSessionStatus === 'READY' ? 'Pagar con Mercado Pago' : 'Retomar pago con Mercado Pago'}
            </Button>
          )}
          {payment?.method === 'MERCADO_PAGO' && !pollingStopped && <p className={styles.formHint}>Confirmando el estado del pago…</p>}
          {payment?.method === 'MERCADO_PAGO' && pollingStopped && paymentPending && (
            <div className={styles.formHint}>
              <p>La confirmación está tardando más de lo esperado.</p>
              <Button variant="ghost" disabled={refreshingPayment || query.isFetching} onClick={() => void manuallyRefreshPaymentStatus()}>
          ...
          {payment?.bankInstructions && (
            <div className={styles.bankDetails}>
              ...
              <span>{payment.bankInstructions.bankName}</span>
              <span>{payment.bankInstructions.accountHolder}</span>
              {payment.bankInstructions.cbu && <span>CBU: {payment.bankInstructions.cbu}</span>}
              {payment.bankInstructions.alias && <span>Alias: {payment.bankInstructions.alias}</span>}
            </div>
          )}
          {payment?.method === 'BANK_TRANSFER' && !payment.receipt && canCancel && (
          ...
          {payment?.receipt && (
            <p className={styles.formHint}>
              Comprobante: {payment.receipt.review === 'APPROVED' ? 'aprobado' : payment.receipt.review === 'REJECTED' ? 'rechazado' : 'en revisión'}
            </p>
          )}
          {canCancel && (
            <Button variant="ghost" className={styles.cancelButton} disabled={busy} onClick={() => setCancelDialogOpen(true)}>
              Cancelar orden
            </Button>
          )}
        </section>
        <section className={`${styles.orderCard} ${styles.orderTimelineCard}`}>
          ...
          {order.timeline.length > 0 ? (
            ...
            <p className={styles.formHint}>Todavía no hay eventos de seguimiento para esta orden.</p>
          )}
        </section>
      </div>
      {order.sellerOrders.length > 0 && (
        <section className={`${styles.orderCard} ${styles.orderSellerOrders}`}>
          <div className={styles.sellerIntro}>
            <p className="eyebrow">Marketplace</p>
            <h2>Entregas por vendedor</h2>
            <p>La orden se actualiza automáticamente a partir de cada suborden.</p>
          </div>
          <div className={styles.sellerOrderCards}>
            {order.sellerOrders.map((sellerOrder) => (
              <article className={styles.sellerOrderCard} key={sellerOrder.id}>
                <div className={styles.sellerOrderCardHeader}>
                  <div>
                    <span className="eyebrow">{sellerOrder.sellerType === 'AFFILIATE' ? 'Afiliado' : 'Tienda'}</span>
                    <h3>{sellerOrder.sellerName}</h3>
                    <p>{sellerOrder.number}</p>
                  </div>
                  <StatusBadge status={statusLabel(sellerOrder.status)} tone={statusTone(sellerOrder.status)} />
                </div>
                <div className={styles.sellerOrderCardContent}>
                  <div>
                    <h4>Productos</h4>
                  ...
                  <div>
                    <h4><Truck size={15} /> Entrega</h4>
                    {sellerOrder.fulfillment.hidden ? (
                      <p>Los datos de entrega se habilitan cuando el pago esté acreditado.</p>
                    ) : sellerOrder.fulfillmentType === 'SHIPMENT' ? (
                      <p>{sellerOrder.fulfillment.addressLine1}, {sellerOrder.fulfillment.city}, {sellerOrder.fulfillment.province}</p>
                    ) : (
                      <p>{sellerOrder.fulfillment.pickupPointName}<br />{sellerOrder.fulfillment.pickupPointAddress}</p>
                    )}
                    {sellerOrder.trackingCode && (
                      <p className={styles.formHint}>Seguimiento: {sellerOrder.carrier ? `${sellerOrder.carrier} · ` : ''}{sellerOrder.trackingCode}</p>
                    )}
                  </div>
                </div>
                ...
                <div className={styles.sellerActions}>
                  {sellerOrder.allowedActions.includes('CONFIRM_RECEIPT') && (
                    <Button variant="secondary" disabled={busy} onClick={() => void confirmSellerDelivery(sellerOrder.id, sellerOrder.version)}>
                      <CheckCircle2 size={16} />Confirmar recepción
                    </Button>
                  )}
                  {sellerOrder.allowedActions.includes('OPEN_ISSUE') && (
                    <Button variant="ghost" disabled={busy} onClick={() => { setIssueTarget(sellerOrder.id); setIssueReason(''); }}>
                      <AlertTriangle size={16} />Abrir reclamo
                    </Button>
                  )}
                </div>
                {sellerOrder.sellerContactPhone && (
                  <p className={styles.formHint}>Contacto del vendedor: {sellerOrder.sellerContactPhone}</p>
                )}
              </article>
            ))}
          </div>
      ...
      <Dialog open={cancelDialogOpen} title="Cancelar orden" description="Esta acción cancelará la orden y liberará las reservas asociadas. ¿Querés continuar?" onClose={() => !busy && setCancelDialogOpen(false)} className={styles.orderCancelDialog}>
      ...
      <Dialog open={Boolean(issueTarget)} title="Abrir reclamo" description="Contanos qué ocurrió con la entrega. El vendedor y la administración recibirán el aviso." onClose={() => !busy && setIssueTarget(null)}>
        <form className={styles.issueForm} onSubmit={(event) => { event.preventDefault(); void submitIssue(); }}>
          <label className={styles.issueField}>
            <span>Motivo</span>
            <textarea value={issueReason} onChange={(event) => setIssueReason(event.target.value)} minLength={3} maxLength={1000} rows={5} placeholder="Describí el problema con la entrega" required />
          </label>
          <div className={styles.orderDialogActions}>
            <Button type="button" variant="secondary" onClick={() => setIssueTarget(null)} disabled={busy}>Cancelar</Button>
            <Button type="submit" disabled={busy || issueReason.trim().length < 3}>{busy ? 'Enviando…' : 'Abrir reclamo'}</Button>
          </div>
        </form>
      </Dialog>
    </div>
