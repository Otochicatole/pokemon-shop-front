...
function NewConversationDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <Dialog open={open} title="Nueva conversación" description="Elegí un cliente y dejá el primer mensaje. La conversación aparecerá en tiempo real en su cuenta." className={styles.adminSupportNewDialog} onClose={resetAndClose}>
    <form className={shared.adminDialogForm} onSubmit={(event) => { event.preventDefault(); submitCreate(); }}>
      {customer ? <div className={styles.adminSupportSelectedCustomer}>
        ...
        {customerQuery.isFetching && <p className={styles.adminSupportSearchState}>Buscando clientes…</p>}
        {customerQuery.isError && <p className="form-error">{adminErrorMessage(customerQuery.error)}</p>}
        {!customerQuery.isFetching && deferredSearch.length >= 2 && customerQuery.data?.data.length === 0 && <p className={styles.adminSupportSearchState}>No encontramos clientes activos.</p>}
        {Boolean(customerQuery.data?.data.length) && <ul className={styles.adminSupportCustomerResults} aria-label="Resultados de clientes">
      ...
      <div className={shared.adminDialogActions}><Button type="button" variant="ghost" onClick={resetAndClose} disabled={create.isPending}>Cancelar</Button><Button type="submit" disabled={!canCreate || create.isPending}><MessageCirclePlus size={16} />{create.isPending ? 'Creando…' : 'Crear chat'}</Button></div>
    </form>
  </Dialog>;
}
...
export function AdminSupportInboxView() {
  ...
  return <>
    ...
    <section className={styles.adminSupportSummary} aria-label="Resumen de soporte">
    ...
    <div className={[shared.adminToolbar, styles.adminSupportToolbar].filter(Boolean).join(' ')}>
    ...
    {query.isLoading ? <div className={shared.adminLoading}>Cargando conversaciones</div> : query.isError ? <div className={shared.adminErrorPanel}><div><h2>No pudimos cargar soporte</h2><p>{adminErrorMessage(query.error)}</p><Button variant="secondary" onClick={() => void query.refetch()}>Reintentar</Button></div></div> : <>
      <AdminDataTable rows={conversations} rowKey={(row) => row.id} caption="Conversaciones de soporte" empty={attention === 'unread' ? 'No hay mensajes nuevos en esta página.' : 'Todavía no hay conversaciones de soporte.'} columns={[
        { key: 'subject', header: 'Conversación', render: (row) => <div className={styles.adminSupportSubject}><Link className={shared.adminTableLink} href={`/admin/support/${row.id}`}>{row.subject}</Link><small className={shared.adminBlockMuted}>{row.lastMessagePreview ?? 'Sin mensajes todavía'}</small></div> },
        { key: 'customer', header: 'Cliente', render: (row) => <div><strong>{row.user.name?.trim() || 'Sin nombre'}</strong><small className={shared.adminBlockMuted}>{row.user.email}</small></div> },
        { key: 'status', header: 'Estado', render: (row) => <SupportStatusBadge status={row.status} /> },
        { key: 'unread', header: 'Nuevos', align: 'center', render: (row) => row.unreadCount > 0 ? <span className={styles.adminSupportUnread} aria-label={`${row.unreadCount} mensajes sin leer`}>{row.unreadCount}</span> : <span className={shared.adminMuted}>—</span> },
        { key: 'activity', header: 'Última actividad', render: (row) => adminDate(row.lastMessageAt, true) },
        { key: 'action', header: 'Abrir', align: 'right', render: (row) => <Link className={shared.adminIconButton} href={`/admin/support/${row.id}`} aria-label={`Abrir conversación ${row.subject}`}><Eye size={17} /></Link> },
      ]} />
