...
export const useCartStore = create<CartState>()(persist((set) => ({
  items: [],
  add: (product, quantity = 1) => set((state) => { const current = state.items.find((item) => item.id === product.id); const next = Math.min(product.available, (current?.quantity ?? 0) + quantity); const item: CartItem = { id: product.id, slug: product.slug, name: product.name, price: product.price, productVersion: product.productVersion, available: product.available, images: product.images, sku: product.sku, seller: product.seller, quantity: next }; return { items: current ? state.items.map((entry) => entry.id === product.id ? item : entry) : [...state.items, item] }; }),
  ...
  clear: () => set({ items: [] }),
  replaceProduct: (product) => set((state) => ({ items: state.items.map((item) => item.id === product.id ? { ...item, name: product.name, slug: product.slug, price: product.price, productVersion: product.productVersion, available: product.available, images: product.images, sku: product.sku, seller: product.seller, quantity: Math.min(item.quantity, product.available) } : item).filter((item) => item.quantity > 0) })),
